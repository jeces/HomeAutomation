...
port="/dev/ttyACM0"    #Temp, Humi, dust, fan,led
port2="/dev/ttyACM1"   #Window
port3="/dev/ttyACM2"
...
######################스레드 끝
    ...
    def pm25(self):
        ...
        while True:
            ...
            list1 = Ardstate.split(',')
            
            self.lcdNumber_pm25.display(list1[2])
            self.lcdNumber_humidity.display(list1[0])
            self.lcdNumber_temperature.display(list1[1])
    ...
    def ser1(self):   #첫번째 아두이노에서 데이터를 보내는것[Temp, Humi, Dust, LED, FAN]
        global Ardstate
        global pm
        while True:
            if ser.readable():
    ...
    def ser2(self): #두번재 아두이노에서 데이터를  보내는것[Window]
        global window_state
        while True:
            if ser2.readable():
    ...
    def ser3(self): #두번재 아두이노에서 데이터를  보내는것[Blind]
        global blind_state
        while True:
            if ser3.readable():
    ...
    def data25(self):
        ...
        if sys.argv[1] == '-s':
            ...
            while 1:
                ...
                strf = str(api_pm25) + "," + str(api_humi) + "," + str(api_temp) + "," + state[0] + "," + state[1] + "," + state[2] + "," + window_state + "," + blind_state + "," + state[3] + "," + state[4]
